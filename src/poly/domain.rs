...
use crate::{
    arithmetic::{best_fft, parallelize, FieldExt, Group, parallelize_count},
    plonk::{Assigned, start_measure, stop_measure}, multicore,
};
...
pub fn butterfly_4_parallel<F: FieldExt>(
    ...
    multicore::scope(|scope| {
        ...
        for (i, (((part0, part1), part2), part3)) in
            part_aa.chunks_mut(chunk)
    ...
    if num_threads > 1 {
        if stage_length == 1 {
            ...
            parallelize_count(data_out, num_threads_recursive, |data_out, i| {
                let num_threads_in_recursion = if num_threads < radix { 1 } else { (num_threads + i) / radix };
                recursive_fft_inner(
                    data_in,
                    data_out,
                    twiddles,
...
impl<F: FieldExt> EvaluationDomain<F> {
    ...
    pub fn new(j: u32, k: u32) -> Self {
        ...
        EvaluationDomain {
            ...
            extended_fft_data: FFTData::<F>::new((1 << extended_k) as usize, extended_omega, extended_omega_inv),
        }
    }
    ...
    pub fn coeff_to_extended(
        ...
        self.fft(&mut a.values, self.extended_omega, self.extended_k);
    ...
    fn op_extended(
        &self,
        mut left: Polynomial<F, ExtendedLagrangeCoeff>,
        right: &Polynomial<F, ExtendedLagrangeCoeff>,
        rotation: Rotation,
        op: impl Fn(&mut F, &F) + Send + Sync + 'static,
    ) -> Polynomial<F, ExtendedLagrangeCoeff>
    where
        F: Field,
    {
    ...
    pub fn mul_extended(
        &self,
        left: Polynomial<F, ExtendedLagrangeCoeff>,
        right: &Polynomial<F, ExtendedLagrangeCoeff>,
        rotation: Rotation,
    ) -> Polynomial<F, ExtendedLagrangeCoeff>
    where
    ...
    fn distribute_powers_zeta(&self, mut a: &mut [F], into_coset: bool) {
    ...
    fn fft_inner(&self, a: &mut Vec<F>, omega: F, log_n: u32, inverse: bool) {
        ...
        let fft_data = if a.len() == self.fft_data.n { &self.fft_data } else { &self.extended_fft_data };
        recursive_fft(fft_data, &mut a_copy, inverse);

        best_fft(a, omega, log_n);

        //println!("{}", inverse);
        for idx in 0..a.len() {
            //println!("{}: {:?} {:?}", idx, a[idx], a_copy[idx]);
            assert_eq!(a[idx], a_copy[idx]);
        }*/
    ...
    fn fft(&self, a: &mut Vec<F>, omega: F, log_n: u32) {
        self.fft_inner(a, omega, log_n, false);
    }
