...
use super::{ConstraintSystem, Expression, start_measure, stop_measure};
...
impl<C: CurveAffine> Evaluator<C> {
    ...
    pub(in crate::plonk) fn evaluate_h(
        ...
        let start = start_measure(format!("cosets"), false);
        ...
        {
            // Custom gates
            let start = start_measure(format!("custom gates"), false);
            ...
            // Permutations
            let start = start_measure(format!("permutations"), false);
