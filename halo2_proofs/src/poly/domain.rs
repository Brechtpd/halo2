...
use crate::{
    ...
    plonk::{Assigned, start_measure, stop_measure, get_time, get_duration}, multicore,
};
...
use std::{marker::PhantomData, env::var};
...
impl<G: Group> EvaluationDomain<G> {
    ...
    pub fn new(j: u32, k: u32) -> Self {
        ...
        EvaluationDomain {
            ...
            extended_fft_data: FFTData::<G::Scalar>::new((1 << extended_k) as usize, extended_omega, extended_omega_inv),
        }
    }
    ...
    /// length.
    pub fn lagrange_to_coeff(&self, mut a: Polynomial<G::Scalar, LagrangeCoeff>) -> Polynomial<G::Scalar, Coeff> {
    ...
    pub fn extended_to_coeff(&self, mut a: Polynomial<G::Scalar, ExtendedLagrangeCoeff>) -> Vec<G::Scalar> {
...
fn test_fft() {
    ...
    let start = start_measure(format!("recursive fft {} ({})", a.len(), num_threads), false);
